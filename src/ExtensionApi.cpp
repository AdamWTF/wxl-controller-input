...
void __cdecl OnUpdate(void*, const void*) { if (g_controller) g_controller->OnUpdate(); }
void __cdecl OnWorldEnter(void*, const void*) { if (g_controller) g_controller->OnWorldEnter(); }
void __cdecl OnWorldLeave(void*, const void*) { if (g_controller) g_controller->OnWorldLeave("world leave"); }
void __cdecl OnInput(void*, const void* raw) {
    if (!g_controller || !raw) return;
    const auto& input = *static_cast<const events::InputArgs*>(raw);
    if (input.message == WM_ACTIVATEAPP) g_controller->OnFocus(input.wparam != 0);
}
void __cdecl DrawPanel(void*) { if (g_controller && g_api) DrawDebugPanel(*g_api, *g_controller); }
...
bool LoadExtension(const WXL_Api* api) noexcept {
    ...
    try {
        ...
        api->Subscribe(static_cast<std::uint32_t>(events::Event::OnWorldEnter), &OnWorldEnter, nullptr);
        api->Subscribe(static_cast<std::uint32_t>(events::Event::OnWorldLeave), &OnWorldLeave, nullptr);
