...
void DrawDebugPanel(const WXL_Api& api, const FeatureController& controller) noexcept {
    ...
    if (const auto* device = controller.CurrentDevice()) {
        std::snprintf(text, sizeof(text), "Device: %s (%s)", device->name.c_str(), device->family.c_str());
        api.UiText(text);
    }
    ...
    std::snprintf(text, sizeof(text), "Left raw: %.3f, %.3f   Right raw: %.3f, %.3f",
    ...
    const auto right = ApplyRadialDeadzone(s.rightX, s.rightY, controller.Settings().cameraDeadzone);
    std::snprintf(text, sizeof(text), "Processed: left %.3f, %.3f   right %.3f, %.3f",
