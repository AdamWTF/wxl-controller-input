...
void MovementController::Update(float x, float y) noexcept {
    ...
    std::array<bool, 4> next{
        value.y < -directionThreshold,
        value.y > directionThreshold,
        value.x < -directionThreshold,
        value.x > directionThreshold,
    };

    // All releases precede all presses, making opposite-direction transitions deterministic.
    for (std::size_t i = 0; i < state_.size(); ++i) {
        if (state_[i] && !next[i]) sink_.SetMovement(static_cast<Movement>(i), false);
    }
    for (std::size_t i = 0; i < state_.size(); ++i) {
        if (!state_[i] && next[i]) sink_.SetMovement(static_cast<Movement>(i), true);
    }
    state_ = next;
}
...
void MovementController::Cancel() noexcept {
    for (std::size_t i = 0; i < state_.size(); ++i) {
        if (state_[i]) sink_.SetMovement(static_cast<Movement>(i), false);
    }
    state_.fill(false);
}
