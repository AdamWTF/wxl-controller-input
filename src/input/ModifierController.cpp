...
ModifierController::ModifierController(float activate, float release)
    : activate_(std::clamp(activate, 0.0F, 1.0F)), release_(std::clamp(release, 0.0F, activate_)) {
}

void ModifierController::UpdateOne(float value, float activate, float release,
                                   bool &state) noexcept {
    if (!state && value >= activate)
        state = true;
    else if (state && value < release)
        state = false;
}
...
void ModifierController::Cancel() noexcept {
    left_ = right_ = false;
}
