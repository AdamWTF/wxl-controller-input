...
BindingMap BuiltInBindings() {
    BindingMap result;
    constexpr std::array<Button, 8> buttons{Button::FaceSouth, Button::FaceEast,
        Button::FaceWest, Button::FaceNorth, Button::DPadUp, Button::DPadRight,
    ...
    const auto addLayer = [&](Layer layer, const auto& slots) {
        for (std::size_t i = 0; i < buttons.size(); ++i) result[{layer, buttons[i]}] = ActionSlot{slots[i]};
    };
    addLayer(Layer::Base, base); addLayer(Layer::LT, lt);
    addLayer(Layer::RT, rt); addLayer(Layer::LTRT, both);
    ...
    return std::visit([](const auto& value) noexcept {
        ...
        if constexpr (std::is_same_v<T, ActionSlot>) {
            ...
            static const std::set<std::string> supported{
                "JUMP", "MOVEFORWARD", "MOVEBACKWARD", "STRAFELEFT", "STRAFERIGHT",
                "TARGETNEARESTENEMY", "TARGETPREVIOUSENEMY",
                "TARGETNEARESTFRIEND", "TARGETPREVIOUSFRIEND",
                "TOGGLEAUTORUN", "TOGGLEGAMEMENU", "TOGGLEWORLDMAP"};
            ...
            return (singleKey || namedKeys.contains(value.key)) && value.modifiers.size() <= 3 &&
...
void BindingController::Update(Button button, bool down, Layer layer) noexcept {
    ...
    if (down) {
        ...
        if (found == bindings_.end() || !IsValid(found->second)) return;
        if (sink_.Press(found->second)) owned = found->second;
    } else if (owned) {
        sink_.Release(*owned);
        owned.reset();
    }
...
void BindingController::Cancel() noexcept {
    for (auto& binding : owned_) {
        if (binding) sink_.Release(*binding);
        binding.reset();
    }
}
